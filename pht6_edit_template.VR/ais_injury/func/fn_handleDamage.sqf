...
if (_bodypart == "") then {
	...
	_dmg = _unit getHit _bodypart; //FIXME: Do we need more tests?
	_damage = _damage - _dmg;
};

/* Since fire starts to "damage" with minimal damage at a large radio, reduce anything that is 
   less than 1e-7 to zero */
if (_damage < 1e-7) then { _damage = 0; };
...
if (tcb_ais_impactEffects && _damage > 0) then {
...
if (!(_unit getVariable "tcb_ais_agony") && {alive _unit}) then {
	...
	/* Rambofactor was needed because of the wrong damage handler */
	_return = _damage;
